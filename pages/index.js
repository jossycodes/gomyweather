...
export default function Weather() {
  ...
  const success = obj => {
    ...
    fetcher(`/api/weather-data-fetch`,
      {
        method: 'POST',
  ...
  return (
    ...
    <Head>
     ...
     <div id="nav" className="uk-padding-small-top uk-padding-remove-bottom uk-box-shadow-medium">
      ...
      <div className="uk-grid uk-grid-collapse">
      <div className="uk-width-auto">
   <button className={((myLon == long && myLat == lat && myLon !== null)? 'active': '')+ ' uk-button uk-button-default uk-padding-small  bi-geo-alt-fill  uk-border-rounded flex-center'} style={ { fontSize: '1.6em',
      color: 'var(--color)' }} onClick={getPermisson}></button>
    </div>
        <div className=" uk-width-expand  flex-center">
        <form onSubmit={handleSubmit}>
        <div className="uk-inline">
        <span className="uk-form-icon uk-form-icon-flip bi-search"></span><input id='city' type="search" className="uk-input uk-width-expand uk-border-rounded" placeholder="Enter a city name" value={city} onChange={(e) => setCity(e.target.value)} style={ { background: 'transparent',
        color: "var(--color)" }} />
      </div>
      </form>
    </div>
         <div className="uk-width-auto uk-padding-small-left">
      {(safe)?  (<button className={`uk-button uk-button-default flex-center  bi-${(theme == 'dark')? 'sun': 'moon-stars'}-fill uk-border-rounded`} style={ { fontSize: '1.6em', color: "var(--color)" }} onClick={(e) => {(theme == 'dark')? setTheme('light'): setTheme('dark')}}></button>): (<div className='uk-padding-small'></div>
      )}
    </div>
    <div className="uk-width-auto  uk-padding-small-left">
   ...
   <div uk-dropdown id="dropdown">
    ...
    <li className="uk-nav-header">Units</li>
     ...
     <div className="uk-container uk-padding-small">
            <div className="uk-grid-row-large uk-grid-column-small uk-grid-match uk-grid uk-child-width-1-2@s   ">     
      <div className="uk-margin-top">
      <div className="uk-card uk-box-shadow-medium uk-border-rounded uk-padding-small weather-card" id="mainCard">
